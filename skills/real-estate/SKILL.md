...
| BEA Regional *(optional, free key)* | county per-capita personal income + 5-yr CAGR — runs only if `BEA_API_KEY` is set |
| DOT HPMS AADT | busiest road's traffic count `area_max_aadt` (within ~1 mi) and `frontage_aadt` (within ~150 m), each with road name + functional class — "is this on a busy road?" |
| EPA Superfund (EnviroFacts FRS / SEMS) | `superfund_site_count` + representative `superfund_sites` names — CERCLIS/Superfund sites in the property's county (county-wide, incl. archived) |
...
| Redfin sold comps | 3–6 nearby sold properties (≤12 mo, ±30% sqft, ±1 bed) ranked by similarity. Falls back to relaxed filter if strict yields <3. |
...
|---|---|
| `1-snapshot.md` | Address, list price, beds/baths/sqft/lot/year, key features, DOM. **Show Redfin's price history table verbatim** (from `redfin_price_history` wiki fact) when available — it's the single best anchor for "is the asking price reasonable", and TX-non-disclosure rows still tell you a sale happened. **Cross-check** list price against `tax_market_value` from the CAD — flag any > 30% gap. **Show the implied historical appreciation** (`redfin_implied_list_appreciation.implied_annual_rate`) if present — this anchors the buy-and-hold projection in actual same-property history, not generic market assumptions. |
| `2-comps.md` | **Prefer the `comparable_sales` fact** in the wiki (deterministic Redfin sold comps within ~1 mi, last 12 mo, ±30% sqft, ±1 bed). Compute $/sqft median + range, give an FMV estimate. Flag list-vs-FMV gap. If the fact is absent (non-disclosure state, rural / thin coverage), fall back to WebSearch. Cite `comparable_sales_filter_tier` so the reader knows whether strict or relaxed comps were used. |
...
| `5-risk.md` | FEMA flood zone (from wiki). **Storm history** — pull `storm_tornado_ef1plus_10yr_count`, `storm_hail_15in_plus_10yr_count`, `storm_wind_58mph_plus_10yr_count` from wiki and translate into insurance/roof-replacement implications (hail is the #1 driver of TX homeowner claims). **Seismic** — `seismic_pga_2pct_50yr` interpreted: <0.1g low, 0.1–0.3g moderate, >0.3g high. **Traffic/noise** — `frontage_aadt` (road essentially at the parcel) and `area_max_aadt` (busiest road within ~1 mi) with their `*_road` / `*_class`: a frontage AADT in the tens of thousands or an Interstate/arterial within 150 m signals road noise and resale drag; near-zero frontage AADT confirms a quiet residential street. **Superfund** — if `superfund_site_count` > 0, name the nearest sites from `superfund_sites` and flag for environmental due diligence, but note the count is county-wide and includes archived/minor CERCLIS entries (treat as "look closer", not "contaminated"); 0 is a clean signal. |
| `6-investment.md` | Buy-and-hold 7-yr (from `computed.json`), BRRRR feasibility, flip math. **Always show the full appreciation-sensitivity table from `computed.json["buy_hold"]["sensitivity"]`** — final value / net proceeds / total return / IRR at 3% / 5% / 7% / 10%. A single point estimate misleads when readers anchor on COVID-era doubling (2020–2022 sunbelt was ~10%/yr; that is not a forward baseline). The table lets the reader pick the rate they believe and read the answer. **Anchor the rate choice in the property's own history**: if `redfin_implied_list_appreciation` is present, point at it — "this property has appreciated at X%/yr over Y years; the 5–7% sensitivity row is the realistic base case". **Use CAD owner data** — if `owner_name` shows the same individual for 10+ years and the property is way under-improved vs. neighbors, that's a value-add candidate. |
